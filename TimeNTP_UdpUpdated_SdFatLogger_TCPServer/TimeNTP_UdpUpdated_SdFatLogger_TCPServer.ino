...
void loop()
{
  ...
  /*  We don't need all this Client fuss if all we want is to send the
  output to all clients connected!!!
  
  // disconnect from previous clients
  if (telnet_client.available()) {
    Serial.println("available");
    telnet_client.flush();
    Serial.println(telnet_client.read());
  }
  if (!telnet_client.connected())
  {
    ...
    telnet_client = server.available();
  }
