...
//  ### NTC temperature measurement values:
...
float Temperature(float AnalogValue,int OutputUnit,float B,float T0,float R0,float R_Balance) {
  // Temperature function outputs float, the actual temperature
  // Temperature function inputs
  // 1.AnalogValue - The analog value (smoothed value between 0.0 and 1023.0)
  ...
  R = (1023.0f*R_Balance/AnalogValue) - R_Balance;
...
void loop()
{
  ...
  for(int i = 0; i < N_ADC; i++) {
    ...
    bout << ',' << Temperature(adc_values_ema[i],T_CELSIUS,EPCOS_G1550_10k,R_BALANCE);
  }
  bout << endl;
